 ...
 */
function renderArchiveNavigationTags(slidesContainerElem, versions) {
    const anchorElemTemplate = document.createElement('a');
    anchorElemTemplate.rel = 'nofollow';
    slidesContainerElem.append(...versions.map((item, index) => {
 ...
 */
function setLeftButtonState(slidesContainerElem) {
    const leftButton = slidesContainerElem.closest('.slider').getElementsByClassName('slider-button-left');
    leftButton.disabled = leftButton.offsetTop > slidesContainerElem.offsetTop;
}
//# sourceMappingURL=archiveNav.js.map